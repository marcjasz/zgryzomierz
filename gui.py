...
class App:
    ...
    def run(self):
        ...
        uploadInfo.pack(side=tk.LEFT, expand=True)
        self.scaleEntry = tk.Entry(uploadFrame2, width=30)
        self.scaleEntry.pack(side=tk.LEFT, expand=True)
        ...
        uploadName.pack(side= tk.LEFT, expand=True)
        self.nameEntry = tk.Entry(uploadFrame3, width=30)
        self.nameEntry.pack(side= tk.LEFT, expand=True)
        ### Video Frame
        ...
        self.canvas = tk.Canvas(self.videoFrame, width=600, height=300)
        self.canvas.pack(anchor=tk.NW)
        self.btn_snapshot = tk.Button(self.videoFrame, text="Zrzut ekranu", width=30, command=self.snapshot)
    ...
    def add_buttons(self):
        self.number = 0
        self.btn_start_video = tk.Button(self.videoFrame, text="Włącz film", width=30, height=1, command=lambda: self.start_video())
        self.btn_start_video.pack(anchor=tk.CENTER, expand=True)
        self.btn_stop_video = tk.Button(self.videoFrame, text="Wyłącz film", width=30, height=1, command=lambda: self.video_stop())
        self.btn_stop_video.pack(anchor=tk.CENTER, expand=True)
        self.btn_start_analyze = tk.Button(self.videoFrame, text="Włącz analizę filmu", width=30, height=1, command=lambda: self.start_recording_video())
        self.btn_start_analyze.pack(anchor=tk.CENTER, expand=True)
        self.btn_stop_analyze = tk.Button(self.videoFrame, text="Wyłącz analizę filmu", width=30, height=1, command=lambda: self.stop_recording_video())
        self.btn_stop_analyze.pack(anchor=tk.CENTER, expand=True)
        ...
        self.pdfButton = tk.Button(self.resultFrame, text="Zapisz analizę", command=lambda: self.create_pdf(), height=1,
                              width=35)
        self.pdfButton.pack(anchor=tk.SW, expand=True)
    ...
    def show_graph(self):
        if self.scaleEntry.get():
            ...
            self.resultMiliLabel['text'] = "Wynik w milimetrach " + str(round(max(self.data_distance[self.films[-1][0]:self.films[-1][1]]),3))
    ...
    def create_graph(self, one, start, stop):
        ...
        if one:
            ...
            axs.invert_yaxis()
            axs.set_title('Odchylenie toru odwodzenia żuchwy', fontsize=12)
            axs.set_ylabel("Kąt[°]", fontsize=8)
            axs.yaxis.grid(color='gray', linestyle='dashed')
        else:
            fig, axs = plt.subplots(2)
            fig.suptitle('Odchylenie toru odwodzenia żuchwy')
            axs[0].plot(y[start:stop], self.data_angle[start:stop])
            axs[0].yaxis.grid(color='gray', linestyle='dashed')
            axs[0].set_ylabel("Kąt[°]", fontsize=8)
    ...
    def start(self):
        if self.answer != '':
            if len(self.data_angle) > 0:
                self.resultCanvas.get_tk_widget().destroy()
                self.btn_start_analyze.destroy()
                self.btn_start_video.destroy()
                self.btn_stop_video.destroy()
                self.btn_stop_analyze.destroy()
                self.resultAngleLabel.destroy()
                self.resultMiliLabel.destroy()
                self.pdfButton.destroy()
            self.finish = queue.Queue()
            self.queue_frame = queue.Queue()
            self.data_frame = []
            self.queue_angle = queue.Queue()
    ...
    def create_thread(self):
        capture = CaptureManager(cv2.VideoCapture(self.answer), self.queue_frame)
...
if __name__ == "__main__":
    program = App(tk.Tk(), "Analiza odchylenia toru odwodzenia żuchwy")
